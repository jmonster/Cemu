...
class DesktopLifecycle(unittest.TestCase):
    ...
    def test_production_cmake_gates(self):
        ...
        # platform, native, SDL, bundle, deployment, SDK exists, expected admission/error
        cases = [
            ('Darwin', False, False, False, '13.4', True, False, None),
            ('FreeBSD', False, False, False, '', False, False, None),
            ('Darwin', True, True, True, '15.0', True, True, None),
            ('Darwin', True, True, False, '15.0', True, False, 'MACOS_BUNDLE'),
            ('Darwin', True, True, True, '13.4', True, False, '15.0'),
            ('Linux', True, True, False, '', True, True, None),
            ('Linux', True, False, False, '', True, False, 'ENABLE_SDL'),
            ('Linux', True, True, False, '', False, False, 'Switch2Kit is missing'),
            ('FreeBSD', True, True, False, '', True, False, 'desktop host'),
        ]
        with tempfile.TemporaryDirectory(prefix='cemu-cmake-policy-') as directory:
            ...
            for platform, native, sdl, bundle, version, exists, admitted, error in cases:
                with self.subTest(platform=platform, native=native, sdl=sdl,
                    ...
                    values = dict(APPLE=platform == 'Darwin', WIN32=False,
...
''' + policy)
                    ...
                    self.assertEqual(result.returncode == 0, error is None, output)
